...
class EventLogEsRepository(BaseRepository[EventLogDoc]):
    ...
    async def delete_by_filters(
        ...
        try:
            ...
            logger.debug(
                "✅ Batch deleted event log by filter conditions: user_id=%s, group_id=%s, deleted %d records",
                user_id,
                group_id,
                deleted_count,
            )
            ...
            logger.error(
                "❌ Failed to batch delete event log: user_id=%s, group_id=%s, error=%s",
                user_id,
                group_id,
                e,
            )
    ...
    async def delete_by_parent_id(self, parent_id: str, refresh: bool = False) -> int:
        """
        Delete event log documents by parent_id.
        ...
        try:
            ...
            deleted_count = response.get('deleted', 0)
            logger.debug(
                "✅ Deleted event log by parent_id: parent_id=%s, deleted %d records",
                parent_id,
                deleted_count,
            )
            return deleted_count
            ...
            logger.error(
                "❌ Failed to delete event log by parent_id: parent_id=%s, error=%s",
                parent_id,
                e,
            )
            raise
